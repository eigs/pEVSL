...
int chebxPltd(int m, double *mu, int npts, double *xi, double *yi) {
  ...
  PEVSL_MALLOC(vkm1, n, double);
  PEVSL_MALLOC(vkp1, n, double);
  PEVSL_MALLOC(vk, n, double);
  double *tmp;
  //-------------------- compute p(xi)
...
void chext(pevsl_Polparams *pol, double aIn, double bIn){
  ...
  PEVSL_CALLOC(t0, work_size, double);
  t1 = t0+m1;
  tnew = t1+m1;
  // t0 = 1
  t0[0] = 1.0;
...
int pEVSL_FindPol(double *intv, pevsl_Polparams *pol) {
  ...
  PEVSL_MALLOC(mu, max_deg+1, double);
  pol->mu = mu;
  PEVSL_MALLOC(v, max_deg+1, double);
  PEVSL_MALLOC(jac, max_deg+1, double);
